...
static inline bool isNoData( double val )
{
  return std::isnan( val );
}

static inline bool equals( double val0, double val1, double eps = std::numeric_limits<double>::epsilon() )
{
  ...
  for ( const auto &ds : vals )
  {
    ...
    if ( ds->datasetCount() > 1 )
    {
      ...
      for ( int datasetIndex = 0; datasetIndex < ds->datasetCount(); ++datasetIndex )
      {
        ...
        if ( timesPopulated )
        {
          if ( !equals( mTimes[datasetIndex], o->time, 1e-5 ) )
          {
...
void QgsMeshCalcUtils::tranferDatasets( QgsMeshMemoryDatasetGroup &group1, QgsMeshMemoryDatasetGroup &group2 ) const
{
  Q_ASSERT( isValid() );
                              ...
                              const QgsMeshMemoryDatasetGroup &condition ) const
{
  ...
  for ( int time_index = 0; time_index < trueGroup.datasetCount(); ++time_index )
  {
    ...
    for ( int n = 0; n < true_o->values.size(); ++n )
    {
      ...
      if ( !isNoData( conditionValue ) )
      {
        if ( equals( conditionValue, D_TRUE ) )
...
) const
{
  ...
  for ( int idx = 0; idx < mMeshLayer->dataProvider()->faceCount(); ++idx )
  {
    ...
    {
      if ( isNoData( dataset->values[face[j]].scalar() ) )
      {
...
{
  Q_ASSERT( !isNoData( val1 ) );

  if ( equals( filter, D_TRUE ) )
    return val1;
  else
    return D_NODATA;
}
...
double QgsMeshCalcUtils::flogicalAnd( double val1, double val2 ) const
{
  Q_ASSERT( !isNoData( val1 ) );
  Q_ASSERT( !isNoData( val2 ) );
  bool bval1 = equals( val1, D_TRUE );
  bool bval2 = equals( val2, D_TRUE );
  if ( bval1 && bval2 )
    return D_TRUE;
  else
    return D_FALSE;
