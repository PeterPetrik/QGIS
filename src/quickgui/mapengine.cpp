...
void MapEngine::setLayers(const QList<QgsMapLayer *> layers)
{
  ...
  QgsRectangle layersExtent;
  foreach (QgsMapLayer* ml, layers)
  {
      qDebug() << "layer loaded " << ml->name() << " | " << ml->isValid();
      if (ml->isValid()) {
          layersExtent.combineExtentWith(ml->extent());
      }
      connect(ml, SIGNAL(repaintRequested()), this, SLOT(onRepaintRequested()));
  }

  qDebug() << "setting initial extent " << layersExtent.toRectF();
  mView->fromExtent(layersExtent.toRectF());
