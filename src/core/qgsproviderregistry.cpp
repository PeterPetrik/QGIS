...
void QgsProviderRegistry::init()
{
  ...
  for ( const QFileInfo &fi : constEntryInfoList )
  {
    ...
    {
      QgsDebugMsg( QStringLiteral( "Checking %1: ...invalid (key %2 already registed)" ).arg( myLib.fileName() ).arg( meta->key() ) );
      delete meta;
      continue;
    }
  ...
  for ( Providers::const_iterator it = mProviders.begin(); it != mProviders.end(); ++it )
  {
    ...
    QString fileVectorFilters = meta->fileVectorFilters();
    ...
    QString fileRasterFilters = meta->fileRasterFilters();
    ...
    meta->fileMeshFilters( fileMeshFilters, fileMeshDatasetFilters );
