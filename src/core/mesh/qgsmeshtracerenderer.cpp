...
QgsMeshStreamField::QgsMeshStreamField( const QgsTriangularMesh &triangularMesh, const QgsMeshDataBlock &dataSetVectorValues, const QgsMeshDataBlock &scalarActiveFaceFlagValues, const QgsRectangle &layerExtent, double magMax, bool dataIsOnVertices, int resolution ):
...
void QgsMeshStreamField::addRandomTraces( int count )
{
...
void QgsMeshStreamField::addGriddedTraces()
{
  int fieldSpacing;
...
void QgsMeshStreamField::addTracesFromBorder()
{
  int fieldSpacing;
  if ( mPixelFillingDensity <= 0 )
  {
...
void QgsMeshStreamField::addTrace( QPoint startPixel )
{
  ...
  while ( !end )
  {
    ...
    double Vx = vu.x();
    double Vy = vu.y();
    double Vu = mag / mMagMax * 2; //nondimentional vector magnitude
    ...
    else
    {
      ...
      dt += dl / Vu ; //adimentional time step : this the time needed to go to the border of the pixel
