...
void TestQgsMeshCalculator::dualOp_data()
{
  ...
  QTest::newRow( "opDIV by 0" ) << QgsMeshCalcNode::opDIV << 2.5 << 0.0 << -9999.0;
  ...
  QTest::newRow( "opPOW complex" ) << QgsMeshCalcNode::opPOW << -2.0 << 0.5 << -9999.0;
...
void TestQgsMeshCalculator::dualOp()
{
  ...
  for ( int i = 0; i < ds->values.size(); ++i )
    QCOMPARE( ds->values.at( i ), expected );
}
